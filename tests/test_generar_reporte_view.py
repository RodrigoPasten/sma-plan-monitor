...
class GenerarReporteViewTest(unittest.TestCase):
    def setUp(self):
        super().setUp()
        self.client = Client()
        ...
        self.url = reverse(
            ...
            kwargs={"tipo_id": self.tipo_reporte.id}
        )

    def test_generar_reporte_access(self):
        resp = self.client.get(self.url)
        self.assertEqual(resp.status_code, 200)
        self.assertIn("tipo_reporte", resp.context)
        self.assertIn("organismos", resp.context)
        self.assertIn("componentes", resp.context)
        self.assertEqual(resp.context["tipo_reporte"], self.tipo_reporte)
